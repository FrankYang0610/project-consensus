...
def seed_forum_data(apps, schema_editor):
    ...
    main_post = ForumPost.objects.create(
        ...
        tags=["opera", "rossini", "classical-music", "comedy", "barber-of-seville", DEMO_TAG],
        likes_count=0,
    )
...
def unseed_forum_data(apps, schema_editor):
    # Clear data generated by this migration script ONLY.
    # Never delete non-demo content during rollback.
    ForumPost = apps.get_model("forum", "ForumPost")
    ForumPostComment = apps.get_model("forum", "ForumPostComment")
    demo_posts = ForumPost.objects.filter(tags__contains=[DEMO_TAG])
    demo_post_ids = list(demo_posts.values_list("pk", flat=True))
    if demo_post_ids:
        ForumPostComment.objects.filter(post_id__in=demo_post_ids).delete()
        ForumPost.objects.filter(pk__in=demo_post_ids).delete()
