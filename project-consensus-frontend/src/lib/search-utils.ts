...
/**
 * Build localized title for search results based on metadata
 * @param title - Raw title from backend
 * @param metadata - Result metadata containing title_template if needed
 * @param t - Translation function
 * @returns Localized title
 */
export function buildSearchResultTitle(
  title: string, 
  metadata: Record<string, string | number | null | undefined>, 
  t: (key: string, params?: Record<string, string | number>) => string
): string {
  const titleTemplate = metadata.title_template;
...
/**
 * Translate author name for anonymous users
 * @param author - Raw author name from backend
 * @param t - Translation function
 * @returns Translated author name
 */
export function translateAuthorName(author: string, t: (key: string) => string): string {
  if (author === 'Anonymous') {
